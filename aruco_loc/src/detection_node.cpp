...
void ImageDetectionNode::stereo_target_detect(cv_bridge::CvImageConstPtr left_img, cv_bridge::CvImageConstPtr right_img) {
    ...
    if (left_mark_it == left_markerIds.end()) {
        ...
        // f is in pixels, so we do not need to convert image coordinates (that are in pixels)
        double z = B*f / (std::abs(left_center.x - right_center.x) * 1000.);
        pt_msg.point.x = ((left_center.x - this->cx) * z) / f;
        pt_msg.point.y = ((left_center.y - this->cy) * z) / f;
        pt_msg.point.z = z;
